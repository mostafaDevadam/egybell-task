                ...
                <table data-testid="table" className="min-w-full table-fixed divide-y divide-gray-200 dark:divide-gray-700 shadow-sm">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-900 dark:divide-gray-700">
                        ...
                        {docs && docs.map((m, index) => (
                            m.id !== user?.id &&
                            <tr key={index} className="text-sm text-gray-500 dark:text-gray-300 dark:hover:bg-gray-700 dark:hover:*:text-white hover:bg-gray-300 hover:text-gray-700">
                                {
                                    fields && fields.map((f, index) => (
                                                    ...
                                                    : isActions && role === 'admin' ? (
                                                        <div className="flex flex-col md:flex-row gap-2">
                                                            <Link to={`/${locale}/profile/${m.id}/view`} data-testid="view"
                                                                className="cursor-pointer text-blue-700 border border-blue-700 hover:border-0 hover:bg-blue-500 hover:text-white px-3 py-1 rounded">
                                                                {t("table.view")}</Link>
                                                            <Link to={`/${locale}/profile/${m.id}/edit`} data-testid="edit"
                                                                className="cursor-pointer text-green-700 border border-green-700 hover:border-0 hover:bg-green-500 hover:text-white px-3 py-1 rounded">
                                                                {t("table.edit")}</Link>
                                                            {role === Role.ADMIN &&
                                                                <RenderActionButton row={m} title={t("table.delete")} className="" />
                                                            }
                                                        </div>
                                                    ) : m[f]}
                                        </td>
